...
foreach ($tbTables as $k => $v) {
    $tabColTitle = array("cid" => "บัตรประชาชน", "cname" => "ชื่อ-สกุล",
        "sotype" => "สถานะภาพ", "soin" => "วันที่เข้าทหาร", "soexp" => "วันที่ปลดทหาร", "sosite" => "สังกัดทหาร", "sonote" => "หมายเหตุ", "");
    $tb_name = $k;
    $tb_title = $v;
        ...
        if ($v != "" && $k != 'cname') {
            switch ($k) {
                case "cid":
                    ...
                    break;
                case "sotype":
                    $arPstype = getMYSQLValueAll('hr_stype', 'id,name');
    ...
    $datadb = getMYSQLValueAll("{$tb_name} as x , hr_person as y ",
        ...
        " (select name from hr_stype as ps where ps.id = x.sotype ) as pstype , x.soin,x.soexp,x.sosite,x.sonote,x.id",
